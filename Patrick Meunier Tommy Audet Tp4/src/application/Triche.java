...
public class Triche
{
	...
	private static void tricheMonter(List<Carte> colonneCartes)
	{
		...
		int i = 0;
		int plusGrand = 0;
		while(i < colonneCartes.size() && temp)
		{
			if(colonneCartes.get(i).getValeur().getValeur() == 1)
			{
				plusGrand = i;
				temp = false;
			}
			...
			{
				if(colonneCartes.get(plusGrand).getValeur().getValeur() >= colonneCartes.get(i).getValeur().getValeur())
				{
					i++;
				}
				else
		...
		tempo = colonneCartes.remove(plusGrand);
		colonneCartes.set(colonneCartes.size() - 1 , tempo);	
	}
