...
public class AcesUpSolitaire extends JFrame
{
	...
	{
		Carte temp = (Carte) getColonneCartes(indexColonne).remove(getColonneCartes(indexColonne).size() - 1);
		int i = indexColonne - 1;
		while(i != indexColonne || !getColonneCartes(i).isEmpty())
		{
			i--;
			i = i % NBR_COLONNES_DE_CARTES;
		}
		if(getColonneCartes(i).isEmpty())
		getColonneCartes(i).add(temp);
	}
	...
	public void gestionEnleverListe(int pNoListe)
	{
		...
		for(int i = 0 ; i < NBR_COLONNES_DE_CARTES ; i++)
		{
			if(i == 0)
			{
				temp = (Carte)(getColonneCartes(i).get(getColonneCartes(i).size() - 1));
			}
			else if(temp.compareTo((Carte)(getColonneCartes(i).get(getColonneCartes(i).size() - 1))) < 0)
			{
				temp = (Carte)(getColonneCartes(i).get(getColonneCartes(i).size() - 1));
				tempo = i;
			}
		...
		if(tempo != pNoListe)
		{
			getColonneCartes(tempo).remove(getColonneCartes(tempo).size() - 1);
		}	
	}
	...
	{
		int i = 0;
		while(i < NBR_COLONNES_DE_CARTES && !pioche.isEmpty())
		{
			getColonneCartes(i).add((Carte)pioche.piger());
			i++;
		}
	}
